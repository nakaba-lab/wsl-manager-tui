...
mod tests {
    ...
    fn gauge_spans_clamps_and_colors() {
        let over: String = gauge_spans(2.0, 4)
            .iter()
            .map(|s| s.content.as_ref())
            .collect();
        assert_eq!(over, "▕████▏");
        assert_eq!(
            gauge_spans(0.5, 4)[1].style.fg,
            Some(theme::gauge_color(0.5))
        );
        assert_eq!(
            gauge_spans(0.8, 4)[1].style.fg,
            Some(theme::gauge_color(0.8))
        );
        assert_eq!(
            gauge_spans(0.95, 4)[1].style.fg,
            Some(theme::gauge_color(0.95))
        );
    }
